...
class DataStoreManager(private val context: Context) {
    ...
    //Websiteblocking
    suspend fun addBlockedWebsite(website: String) {
        val current = if (appSettings.value.accountMode == "Parent") {
            appSettings.value.blockedWebsites
        } else {
            appSettings.value.childProfiles.find { it.id == appSettings.value.activeChildId }?.blockedWebsites ?: emptyList()
        }

        val updated = current + website
        setBlockedWebsites(updated.distinct())
    }

    suspend fun removeBlockedWebsite(website: String) {
        val current = if (appSettings.value.accountMode == "Parent") {
            appSettings.value.blockedWebsites
        } else {
            appSettings.value.childProfiles.find { it.id == appSettings.value.activeChildId }?.blockedWebsites ?: emptyList()
        ...
        val updated = current.filterNot { it.equals(website, ignoreCase = true) }
        setBlockedWebsites(updated)
    }


    suspend fun setBlockedWebsites(websites: List<String>) {
        if (appSettings.value.accountMode == "Parent") {
            saveSettings { it.copy(blockedWebsites = websites) }
        } else {
            updateActiveChild { it.copy(blockedWebsites = websites) }
        }
    }

    fun getBlockedWebsites(): StateFlow<List<String>> {
    ...
    fun isWebsiteBlocked(url: String): Boolean {
        ...
        val blocked = if (settings.accountMode == "Parent") {
            settings.blockedWebsites
        } else {
            settings.childProfiles.find { it.id == settings.activeChildId }?.blockedWebsites ?: emptyList()
        }
        return blocked.any { url.contains(it, ignoreCase = true) }
    }
...
// ------------------- CATEGORY BLOCKING (Parent or Child) -------------------
    ...
    // ✅ Get bedtime blocks for current mode
    fun getBedtimeBlocks(): StateFlow<List<BedtimeBlock>> {
        return appSettings.map { settings ->
            if (settings.accountMode == "Parent") {
                settings.bedtimeBlocks
            } else {
                settings.childProfiles.find { it.id == settings.activeChildId }?.bedtimeBlocks
                    ?: emptyList()
            }
        }.stateIn(scope, SharingStarted.Eagerly, emptyList())
    }
    ...
    // Save bedtime blocks depending on account mode

    // (Optional) If you ever want direct child-specific functions:
    fun getChildBedtimeBlocks(childId: String): Flow<List<BedtimeBlock>> =
        appSettings.map { settings ->
            settings.childProfiles.find { it.id == childId }?.bedtimeBlocks ?: emptyList()
        }

    suspend fun setChildBedtimeBlocks(childId: String, blocks: List<BedtimeBlock>) {
        saveSettings { settings ->
            settings.copy(
                childProfiles = settings.childProfiles.map { child ->
                    if (child.id == childId) child.copy(bedtimeBlocks = blocks) else child
                }
            )
        }
    }
