...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    val bedtimeBlocks: StateFlow<List<BedtimeBlock>> =
        dataStoreManager.getBedtimeBlocks()
            .stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), emptyList())
    ...
    //Website thing in this

    val blockedWebsites: StateFlow<List<String>> = appSettings
        .map { settings ->
    ...
    fun setBlockedWebsites(websites: List<String>) = viewModelScope.launch {
        dataStoreManager.setBlockedWebsites(websites)
    }

    fun addBlockedWebsite(website: String) = viewModelScope.launch {
        dataStoreManager.addBlockedWebsite(website)
    }

    fun removeBlockedWebsite(website: String) = viewModelScope.launch {
        dataStoreManager.removeBlockedWebsite(website)
    }

    fun isWebsiteBlocked(url: String): Boolean {
        return dataStoreManager.isWebsiteBlocked(url)
    }
